...
EXCEL_FILE = "wineroute_wineries.xlsx"
...
def get_all_instructions():
    system_instructions = (
        ...
        "- Extract the single most relevant company email or contact person's work email.\n"
...
def ensure_email_column(ws) -> int:
    headers = get_header_indexes(ws)
    if "Email" in headers:
        return headers["Email"]

    new_col = ws.max_column + 1
    ws.cell(row=1, column=new_col).value = "Email"
    return new_col
...
def main():
    ...
    email_col = ensure_email_column(ws)
    wb.save(EXCEL_FILE)
    ...
    for row in range(2, ws.max_row + 1):
        ...
        if not website_url:
            ws.cell(row=row, column=email_col).value = ""
            wb.save(EXCEL_FILE)
            print(f"Row {row}: No website found. Empty value written.")
            continue
        ...
        try:
            ...
            if email != "null":
                ...
                ws.cell(row=row, column=email_col).value = ""
                wb.save(EXCEL_FILE)
                print(f"Row {row}: No email found for {website_url}. Empty value written.")
